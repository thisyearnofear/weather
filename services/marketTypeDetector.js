...
export class MarketTypeDetector {
  ...
  static analyzeResolutionDate(market) {
    ...
    if (!resolutionDate) {
      return { signal: 'resolution_date', score: 0, detail: 'No resolution date' };
    }
  ...
  static analyzeLanguagePatterns(market) {
    ...
    return {
      ...
      detail: patterns.length > 0 ? `Matched: ${patterns.join(', ')}` : 'No futures patterns'
    };
  }
  ...
  static analyzeOdds(market) {
    ...
    const yesOdds = typeof odds.yes === 'number' ? odds.yes : parseFloat(odds.yes || 0);
    const bid = typeof market.bid === 'number' ? market.bid : parseFloat(market.bid || 0);
    const ask = typeof market.ask === 'number' ? market.ask : parseFloat(market.ask || 0);
    ...
    if (maxOdds > 0 && maxOdds < 0.05) {
      return { signal: 'odds_analysis', score: 2, detail: `Very low odds (${(maxOdds * 100).toFixed(1)}%) suggests multi-outcome futures` };
    } else if (maxOdds > 0 && maxOdds < 0.15) {
      return { signal: 'odds_analysis', score: 1, detail: `Low odds (${(maxOdds * 100).toFixed(1)}%) might indicate futures` };
    }
  ...
  static analyzeMetadata(market) {
    ...
    const tagLabels = tags.map(t => typeof t === 'string' ? t : t.label || '').join(' ').toLowerCase();
    
    // Look for futures-specific tags
    if (tagLabels.includes('futures') || tagLabels.includes('season winner') || tagLabels.includes('championship')) {
      return { signal: 'metadata', score: 3, detail: 'Futures tag detected' };
    }
    
    // Look for specific event tags that indicate single games
    if (tagLabels.includes('game') || tagLabels.includes('match') || tagLabels.includes('tonight')) {
      return { signal: 'metadata', score: -2, detail: 'Single event tag detected' };
    }
